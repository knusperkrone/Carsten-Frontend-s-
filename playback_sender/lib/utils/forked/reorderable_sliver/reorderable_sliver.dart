...
//      '_dragStartIndex:$_dragStartIndex _ghostIndex:$_ghostIndex _currentIndex:$_currentIndex _nextIndex:$_nextIndex isAcceptingNewTarget:$isAcceptingNewTarget isCompleted:${_entranceController.isCompleted}');
    ...
    if (_entranceController.isCompleted) {
      ...
      final Set<int> updateIndexSet = Set<int>();
      ...
      final updateIndexes = updateIndexSet.toList();

      void _setState() {
        if (updateIndexes.isNotEmpty) {
          final int index = updateIndexes.removeLast();
      ...
      if (localizations != null) {
        ...
        if (index > 0) {
          semanticsActions[CustomSemanticsAction(label: localizations.reorderItemToStart)] =
              moveToStart;
          final String reorderItemBefore = localizations.reorderItemUp;
...
//        }
        ...
        if (index < _childCount - 1) {
          final String reorderItemAfter = localizations.reorderItemDown;
//        if (widget.direction == Axis.horizontal) {
...
//      );
    ...
    Widget buildDragTarget(
        BuildContext context, List<int> acceptedCandidates, List<dynamic> rejectedCandidates) {
      final Widget toWrapWithSemantics = wrapWithSemantics();

      final Widget feedbackBuilder = Builder(builder: (BuildContext context) {
...
//            child: _makeAppearingWidget(toWrap)
      ...
      // draggable.
      if (index >= _childCount || !widget.canReorder(index)) {
        child = toWrap;
      }

      final containedDraggable = Builder(builder: (BuildContext context) {
    ...
    return Builder(builder: (BuildContext context) {
      Widget dragTarget = DragTarget<int>(
        ...
        onWillAccept: (int toAccept) {
          final bool willAccept = _dragStartIndex == toAccept && toAccept != index;
...
//      );
      ...
      final Widget spacing = _draggingWidget == null
...
//      );
      ...
      final int shiftedIndex = _shiftIndex(index, _currentIndex, _ghostIndex);

      if (shiftedIndex == _currentIndex || index == _ghostIndex) {
        final Widget entranceSpacing = _makeAppearingWidget(spacing);
        final Widget ghostSpacing = _makeDisappearingWidget(spacing);
...
//    assert(debugCheckHasMaterialLocalizations(context));
    ...
    final _ReorderableSliverChildDelegateMixin reorderableDelegate =
